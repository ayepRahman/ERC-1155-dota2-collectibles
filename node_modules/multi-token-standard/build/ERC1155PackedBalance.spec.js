...
contract('ERC1155PackedBalance', (accounts) => {
    const LARGEVAL = new utils_1.BigNumber(2).pow(256).sub(2); // 2**256 - 2
    ...
    describe('Bitwise functions', () => {
        it('getValueInBin should return expected balance for given types', async () => {
            let expected = new utils_1.BigNumber(2).pow(16).sub(2); // 2**16-2
            let balance = await erc1155Contract.functions.getValueInBin(LARGEVAL.toString(), 15);
            utils_1.expect(balance).to.be.eql(expected);
        });
        ...
        it('getIDBinIndex should return the correct bin and respective index', async () => {
            ...
            utils_1.expect(bin0).to.be.eql(new utils_1.BigNumber(0));
            utils_1.expect(index0).to.be.eql(new utils_1.BigNumber(0));
            const { bin: bin6, index: index6 } = await erc1155Contract.functions.getIDBinIndex(6);
            utils_1.expect(bin6).to.be.eql(new utils_1.BigNumber(0));
            utils_1.expect(index6).to.be.eql(new utils_1.BigNumber(6));
            const { bin: bin16, index: index16 } = await erc1155Contract.functions.getIDBinIndex(16);
            utils_1.expect(bin16).to.be.eql(new utils_1.BigNumber(1));
            utils_1.expect(index16).to.be.eql(new utils_1.BigNumber(0));
            const { bin: bin31, index: index31 } = await erc1155Contract.functions.getIDBinIndex(31);
            utils_1.expect(bin31).to.be.eql(new utils_1.BigNumber(1));
            utils_1.expect(index31).to.be.eql(new utils_1.BigNumber(15));
        });
    });
    ...
    describe('safeBatchTransferFrom() function', () => {
        ...
        describe('TransferBatch event', async () => {
            ...
            it('should have `msg.sender` as `_operator` field, not tx.origin', async () => {
                ...
                // Execute transfer from operator contract
                // @ts-ignore (https://github.com/ethereum-ts/TypeChain/issues/118)
                await operatorContract.functions.safeBatchTransferFrom(erc1155Contract.address, ownerAddress, receiverAddress, types, values, [], { gasLimit: 1000000 } // INCORRECT GAS ESTIMATION
                );
                // Get logs from internal transaction event
                // @ts-ignore (https://github.com/ethers-io/ethers.js/issues/204#issuecomment-427059031)
                filterFromOperatorContract.fromBlock = 0;
...
//# sourceMappingURL=ERC1155PackedBalance.spec.js.map